...
void GSM3ShieldV1ModemCore::manageReceivedData()
{
	if(_debug)
	{
/*		SerialUSB.print(theBuffer().getHead());
		SerialUSB.print(" ");
		SerialUSB.println(theBuffer().getTail());*/
...
//		SerialUSB.println();
//		SerialUSB.print("Com:");
//		SerialUSB.print(ongoingCommand);
//		SerialUSB.print("   Step:");
//		SerialUSB.print(commandCounter);
	}
}
...
void GSM3ShieldV1ModemCore::delayInsideInterrupt(unsigned long milliseconds)
{
	for (unsigned long k=0;k<milliseconds;k++)  
		theGSM3ShieldV1ModemCore.gss.tunedDelay(7000); 
}
